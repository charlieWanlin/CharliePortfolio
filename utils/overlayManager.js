...
class OverlayManager {
  constructor() {
    ...
    // ===== OVERLAY FICHE DE PERSONNAGE (parchemin) =====
    this.ficheOverlay = document.getElementById("fiche-overlay");
    this.ficheBtn = document.getElementById("fiche-close-btn");
  ...
  initEvents() {
    ...
    // ===== BOUTON CONSULTER LA FICHE (dans overlay simple) =====
    ...
    // ===== BOUTON RETOUR DE LA FICHE DE PERSONNAGE =====
    if (this.ficheBtn) {
      this.ficheBtn.addEventListener("click", () => this.fermerFiche());
      this.ficheBtn.addEventListener("touchstart", (e) => {
        e.preventDefault();
        this.fermerFiche();
      });
    }
  ...
  afficherDialogueAvecContinuer(message, objetData = null) {
    ...
    // ===== CAS 1 : PARCHEMIN / FICHE DE PERSONNAGE =====
    // Affiche 2 boutons : "Consulter la fiche" (B) et "Continuer" (A)
    if (nom === "Fiche de personnage" || nom === "Parchemin ancien") {
   ...
   * Bouton B : Ouvrir la fiche de personnage
   * Bouton A : Continuer
   */
  afficherDialogueParchemin(nom, icone, message, image) {
    ...
    this.currentChoices = [
      { key: "B", label: "Consulter la fiche", action: "open-fiche" },
    ...
    this.dialogueFooter.innerHTML = `
      <button class="dialogue-btn" id="btn-lire-lettre">
        <span class="btn-key">Appuyez sur (B) </span> pour lire ma fiche
      </button>
    ...
    setTimeout(() => {
      // ===== ÉVÉNEMENT BOUTON B : CONSULTER LA FICHE =====
  ...
  handleChoice(action) {
    if (action === "continue") {
      ...
      // Ouvrir la fiche de personnage (remplace l'ancienne lettre de motivation)
      this.afficherFiche();
    }
  }
  ...
  // ========================================
  /**
   * Affiche la fiche de personnage en plein écran
   * Ferme d'abord la bulle de dialogue, puis maintient l'état "overlay ouvert"
   * pour que le joueur ne se déplace pas derrière la fiche
   */
  afficherFiche() {
    if (!this.ficheOverlay) {
      return;
    }

    this.fermerDialogue();
    this.simpleOverlay.classList.add("hidden");
    this.ficheOverlay.classList.remove("hidden");
    this.isOverlayOpen = true;
  }

  /**
   * Ferme la fiche de personnage et rend la main au joueur
   */
  fermerFiche() {
    if (!this.ficheOverlay) {
      return;
    ...
    this.ficheOverlay.classList.add("hidden");
    this.isOverlayOpen = false;
    this.currentChoices = null;
    this.currentObject = null;
  }
  ...
  afficherRideau() {
    ...
    if (cvButton) {
      cvButton.addEventListener("click", () => {
        window.open("./files/Charlie_Wanlin_CV.pdf", "_blank");
      });
    }
  }
  ...
  fermerTous() {
    ...
    // Fermer aussi la fiche de personnage si elle est ouverte
    if (this.ficheOverlay) {
      this.ficheOverlay.classList.add("hidden");
    }
