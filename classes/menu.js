...
export class Menu {
  ...
  constructor(canvas, c) {
    ...
    // Overlay Projets
    this.projetsOverlay = document.getElementById("projets-overlay");
    this.projetsBtn = document.getElementById("projets-close-btn");
    ...
    // Événement pour fermer Projets
    if (this.projetsBtn) {
      this.projetsBtn.addEventListener("click", () => {
        this.projetsOverlay.classList.add("hidden");
      });
    }

    // Neutraliser les liens de projets pas encore disponibles
    // (démo non déployée ou dépôt privé) pour éviter un saut de page
    if (this.projetsOverlay) {
      this.projetsOverlay
  ...
  handleClick(e) {
    ...
    if (this.state === "PRINCIPAL") {
      ...
      // Bouton PROJETS → Afficher l'overlay Projets
      if (this.isInside(x, y, this.boutons.projets)) {
        this.projetsOverlay.classList.remove("hidden");
        return;
      }
